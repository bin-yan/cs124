...
void strassenRecursive(vector< vector<int> > &A,
    ...
    if (n <= threshold) {
        ...
        strassenRecursive(a11, temp1, p1, n_new, threshold); // p1 = (a11) * (b12 - b22)

        vector< vector<int> > temp2(n_new,inner);
        sum(a11, a12, temp2, n_new); // a11 + a12
        strassenRecursive(temp2, b22, p2, n_new, threshold); // p2 = (a11+a12) * (b22)

        vector< vector<int> > temp3(n_new,inner);
        sum(a21, a22, temp3, n_new); // a21 + a22
        strassenRecursive(temp3, b11, p3, n_new, threshold); // p3 = (a21+a22) * (b11)
        ...
        strassenRecursive(temp5, temp6, p5, n_new, threshold); // p5 = (a11+a22) * (b11+b22)

        vector< vector<int> > temp7(n_new,inner), temp8(n_new,inner);
        subtract(a12, a22, temp7, n_new); // a12 - a22
        sum(b21, b22, temp8, n_new); // b21 + b22
        strassenRecursive(temp7, temp8, p6, n_new, threshold); // p6 = (a12-a22) * (b21+b22)

        vector< vector<int> > temp9(n_new,inner), temp10(n_new,inner);
        subtract(a11, a21, temp9, n_new); // a11 - a21
        sum(b11, b12, temp10, n_new); // b11 + b12
        strassenRecursive(temp9, temp10, p7, n_new, threshold); // p7 = (a11-a21) * (b11+b12)
        ...
        sum(p1, p2, c12, n_new); // c12 = p1 + p2
        sum(p3, p4, c21, n_new); // c21 = p3 + p4

        vector< vector<int> > temp13(n_new,inner), temp14(n_new,inner);
        sum(p1, p5, temp13, n_new); // p1 + p5
        subtract(temp13, p3, temp14, n_new); // p1 + p5 - p3
        subtract(temp14, p7, c22, n_new); // c22 = p1 + p5 - p3 - p7
